...
class PartlySwapper:
    ...
    Args:
        swapping_minority_ratio (float): The proportion of the minority group's to create via swapping.
    ...
    def __init__(self, swapping_minority_ratio, points_to_swap_range):
        self._swapping_minority_ratio = swapping_minority_ratio
    ...
    def __call__(
        ...
        Returns:
            tuple: A tuple containing the modified encoding vectors and the corresponding spurious labels.
        """
        ...
        minority_count = int(self._swapping_minority_ratio * context_class_size)
        ...
        selected_first_class = np.random.choice(np.where(context_labels == 0)[0], size=minority_count, replace=False)
        selected_sec_class = np.random.choice(np.where(context_labels == 1)[0], size=minority_count, replace=False)
        ...
        temp = context_spurs[selected_first_class].copy() 
        context_spurs[selected_first_class] = context_spurs[selected_sec_class]
        context_spurs[selected_sec_class] = temp

        return image_enc, context_spurs

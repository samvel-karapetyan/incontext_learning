...
from src.datamodules.datasets import Camelyon17ForEncodingExtraction
...
class Camelyon17DataModule(pl.LightningDataModule):
    """
        A PyTorch Lightning data module for the Camelyon17 dataset.

        This module is designed to handle the loading and preprocessing of the Camelyon17 dataset for
    ...
    def prepare_data(self):
        """Prepares the Camelyon17 dataset for use, downloading it if necessary."""
        self._dataset = Camelyon17ForEncodingExtraction(root_dir=self._root_dir, download=True)

    def train_dataloader(self) -> DataLoader:
        """Returns a DataLoader for the training subset of the Camelyon17 dataset."""
    ...
    def val_dataloader(self) -> DataLoader:
        """Returns a DataLoader for the OOD validation subset of the Camelyon17 dataset."""
    ...
    def id_val_dataloader(self) -> DataLoader:
        """Returns a DataLoader for the ID validation subset of the Camelyon17 dataset."""
        val_data = self._dataset.get_subset("id_val", transform=self._transform)
        val_loader = get_eval_loader("standard", val_data,
                                     batch_size=self._batch_size,
                                     num_workers=self._num_workers)
    ...
    def test_dataloader(self) -> DataLoader:
        """Returns a DataLoader for the test subset of the Camelyon17 dataset."""
